...
Now that you have [Botonic and its dependencies installed](/intro/installation), you can build your first bot!
...
* `start` will tell the CLI create a new bot.
* `testBot` will be the directory name and the bot name for your project.
...
This will open an ineractive chat session with your bot. Just type any text and press enter to send a message to the bot. If you started with the "blank" template the bot will always respond with "I don't understand you". To finish the chat session type `/q` or `CTRL-C`.

### Project Structure

* `botonic.config.js`: Main configuration file for your bot. Here you'll define routes, which map user inputs to actions, and integrations with 3rd party services, like NLP platforms.
* `pages`: This is where your React components live, there are 2 types of them:
  * `actions`: Actions are the units of logic that your bot can perform and where the responses of your bot are defined.
...
* `...`: The rest of the files are needed for the [babel](https://babeljs.io/) compiler and [Next.js](https://github.com/zeit/next.js/) configuration. Modify them at your own risk!
...
The first step is to create a route that captures any text. Edit your `botonic.config.js` file and add a new rule `{text: /^.+$/, action: "echo"}` inside the `routes` section:
...
Next, create a new file inside `pages/actions` named "echo.js" and paste this code:
...
Botonic has an NLU module under development, meanwhile, you can use our Dialogflow integration. [Dialogflow](https://dialogflow.com/) is Google's NLU as a service.
...
Note that if you just have to return some static messages it's not necessary to create a full `Botonic.React.Component` subclass. It's enough to export a function that returns the messages in RCML.
